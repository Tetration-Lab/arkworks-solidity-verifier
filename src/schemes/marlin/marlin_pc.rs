...
use ark_ff::{to_bytes, FftField, PrimeField};
use ark_marlin::{IndexVerifierKey, Marlin};
use ark_poly::univariate::DensePolynomial;
use ark_poly_commit::marlin_pc::MarlinKZG10;
use digest::Digest;

use crate::{
    constants::TEMPLATE_PREFIX_TEXT,
    utils::{format_bigint, format_inv, format_modulus},
...
pub struct Proof<E: PairingEngine> {
    pub comms_1: Vec<E::G1Affine>,
    pub comms_2: Vec<E::G1Affine>,
    pub degree_bound_comms_2_g1: E::G1Affine,
    pub comms_3: Vec<E::G1Affine>,
    pub degree_bound_comms_3_g2: E::G1Affine,
    pub evals: Vec<E::Fr>,
    pub batch_lc_proof_1: E::G1Affine,
    pub batch_lc_proof_1_r: E::Fr,
    pub batch_lc_proof_2: E::G1Affine,
}

impl<E: PairingEngine + PairingLibrary, D: Digest> SolidityVerifier<E>
    for Marlin<E::Fr, MarlinKZG10<E, DensePolynomial<E::Fr>>, D>
{
    type Proof = Proof<E>;

    type VerifyingKey = IndexVerifierKey<E::Fr, MarlinKZG10<E, DensePolynomial<E::Fr>>>;

    fn export(vk: &Self::VerifyingKey) -> String {
        let template = String::from(CONTRACT_TEMPLATE);
        let fs_seed = to_bytes![b"marlin", &vk].unwrap();
        let x_root_of_unity =
            <E::Fr as FftField>::get_root_of_unity(vk.index_info.num_instance_variables).unwrap();
        ...
        let src = template
            ...
            .replace("<%vk_kzg_g%>", &E::g1_to_string(&vk.verifier_key.vk.g))
            .replace(
                "<%vk_kzg_gamma_g%>",
                &E::g1_to_string(&vk.verifier_key.vk.gamma_g),
            )
            .replace("<%vk_kzg_h%>", &E::g2_to_string(&vk.verifier_key.vk.h))
            .replace(
                "<%vk_kzg_beta_h%>",
                &E::g2_to_string(&vk.verifier_key.vk.beta_h),
            )
            .replace(
            ...
            .replace("<%vk_g1_shift%>", &{
                let h_domain_size = if vk.index_info.num_constraints.is_power_of_two() {
                    vk.index_info.num_constraints
                } else {
                    vk.index_info.num_constraints.next_power_of_two()
                };
                E::g1_to_string(
                    &vk.verifier_key
                        .degree_bounds_and_shift_powers
            ...
            .replace("<%vk_g2_shift%>", &{
                let k_domain_size = if vk.index_info.num_non_zero.is_power_of_two() {
                    vk.index_info.num_non_zero
                } else {
                    vk.index_info.num_non_zero.next_power_of_two()
                };
                E::g1_to_string(
                    &vk.verifier_key
                        .degree_bounds_and_shift_powers
            ...
            .replace("<%fs_init_seed_len%>", &(fs_seed.len() / 32).to_string())
            .replace("<%fs_init_seed_overflow_len%>", &{
                let seed_len_in_32_byte_words = fs_seed.len() / 32;
                let seed_len_overflow_in_bytes = fs_seed.len() - (seed_len_in_32_byte_words * 32);
                seed_len_overflow_in_bytes.to_string()
            })
            .replace("<%fs_populate_init_seed%>", &{
                let mut populate_init_seed = String::new();
                for i in 0..fs_seed.len() / 32 {
                    let word_32_bytes = hex::encode(&fs_seed[i * 32..i * 32 + 32]);
                    write!(
                        populate_init_seed,
                        "init_seed[{}] = 0x{};",
                        i, &word_32_bytes
            ...
            .replace("<%fs_init_seed_overflow%>", &{
                ...
                format!(
                    "0x{}",
                    hex::encode(&fs_seed[seed_len_in_32_byte_words * 32..])
                )
            })
            .replace("<%h_domain_size%>", &{
                let size = if vk.index_info.num_constraints.is_power_of_two() {
                    vk.index_info.num_constraints as u64
                } else {
                    vk.index_info.num_constraints.next_power_of_two() as u64
                };
                size.to_string()
            })
            .replace("<%k_domain_size%>", &{
                let size = if vk.index_info.num_non_zero.is_power_of_two() {
                    vk.index_info.num_non_zero as u64
                } else {
                    vk.index_info.num_non_zero.next_power_of_two() as u64
                };
                size.to_string()
            })
            .replace("<%x_domain_size%>", &{
                let x = vk.index_info.num_instance_variables;
            ...
            .replace(
                ...
                &vk.index_info.num_instance_variables.to_string(),
            )
            .replace(
                "<%num_public_inputs%>",
                &vk.index_info.num_instance_variables.to_string(),
            )
            .replace("<%x_root%>", &format_bigint(x_root_of_unity.into_repr()))
            .replace("<%f_mod%>", &format_modulus::<E::Fq>())
            .replace("<%f_r%>", &format_modulus::<E::Fr>())
            .replace("<%f_inv%>", &format_inv::<E::Fr>());

        format!("{}\n{}\n{}", TEMPLATE_PREFIX_TEXT, E::template(false), src)
    }
}
